...
app.post("/api/deleteProp", (req, res) => {
  ...
  db.query(sqlDelete, [id], (error, result) => {
    ...
    console.log(result);
    res.status(200).send({ DeleteStatus: true, resultid: id });
  });
  // console.log(res);
});
...
app.get("/api/Props", (req, res) => {
  const sqlSelect =
    "SELECT *, DATE_FORMAT(Date, '%Y-%m-%d %H:%i:%s') AS formatted_date FROM props";
  db.query(sqlSelect, (error, result) => {
    if (error) {
  ...
  db.query(sqlSelect, [email], (error, result) => {
    if (error) {
      ...
      if (result.length) {
        ...
        res
          .status(404)
          .json({ status: 404, message: "Success", ses: req.body });
      }
    }
  });
});
        ...
        db.query(
          ...
          (error, result) => {
            if (error) {
              ...
              res
                .status(404)
                .json({ status: 404, message: "Agent created Successfully" });
            }
          }
        );
